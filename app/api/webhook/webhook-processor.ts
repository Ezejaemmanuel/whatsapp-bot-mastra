...
export function initializeWebhookService(config: WebhookServiceConfig = {}): WebhookServiceInstance {
    try {
        ...
        if (!config.accessToken && !envAccessToken) {
            const error = new Error('Missing required environment variable: WHATSAPP_ACCESS_TOKEN (WhatsApp Business API access token from Meta Business)');
            logError('Failed to initialize WhatsApp Webhook Service', error, {
        ...
        if (!config.phoneNumberId && !envPhoneNumberId) {
            const error = new Error('Missing required environment variable: WHATSAPP_PHONE_NUMBER_ID (WhatsApp Business phone number ID from Meta Business)');
            logError('Failed to initialize WhatsApp Webhook Service', error, {
                operation: 'initializeWebhookService',
 ...
 * This function now catches all errors internally and never throws to ensure HTTP 200 response
 */
export async function processIncomingMessage(
    ...
    try {
        ...
        if (message.from === botPhoneNumber) {
            ...
            return; // Early return is safe here
        }
        ...
        try {
            ...
            try {
                ...
                logWarning('Failed to send error response for extract error', {
                    error: responseError instanceof Error ? responseError.message : String(responseError),
                    operation: 'processIncomingMessage:extract_error_response'
                });
            }
            return; // Safe return instead of throw
        }
        ...
        try {
            ...
            if (conversation && conversation.inCharge === 'admin') {
                ...
                try {
                    ...
                    logWarning('Failed to store admin-handled message', {
                        error: storeError instanceof Error ? storeError.message : String(storeError),
                        operation: 'processIncomingMessage:store_admin_message'
                    });
                }
                return; // Safe return instead of throw
            }
            ...
            try {
                ...
                logWarning('Failed to send error response for user error', {
                    error: responseError instanceof Error ? responseError.message : String(responseError),
                    operation: 'processIncomingMessage:user_error_response'
                });
            }
            return; // Safe return instead of throw
        }
        ...
        try {
            ...
            switch (messageInfo.type) {
                ...
                default:
                    ...
                    try {
                        ...
                        logWarning('Failed to send unhandled message type response', {
                            error: responseError instanceof Error ? responseError.message : String(responseError),
                            operation: 'processIncomingMessage:unhandled_type_response'
                        });
                    }
            }
            ...
            // Send user-friendly error response but don't throw
            try {
                await sendErrorResponse(
                    whatsappClient,
                    messageInfo.from,
                ...
                logWarning('Failed to send processing error response', {
                    error: responseError instanceof Error ? responseError.message : String(responseError),
                    operation: 'processIncomingMessage:processing_error_response'
                });
            }
        }
        ...
        // Try to send an error response but don't throw even if it fails
        try {
            await sendErrorResponse(
                whatsappClient,
                message.from,
            ...
            logError('Failed to send final error response', responseError as Error, {
                messageId: message.id,
                from: message.from,
                operation: 'processIncomingMessage:final_error_response'
            });
        }

        // Never throw - let the main route return HTTP 200
    }
}
        ...
        for (const entry of payload.entry) {
            for (const change of entry.changes) {
                try {
                    ...
                    if (change.value.messages) {
                        for (const message of change.value.messages) {
                            try {
                                ...
                                logError('Error processing individual message', messageError as Error, {
                                    messageId: message.id,
                                    from: message.from,
                                    operation: 'processWebhookPayload:individual_message'
                                });
                    ...
                    if (change.value.statuses) {
                        for (const status of change.value.statuses) {
                            try {
                                ...
                                logError('Error processing status update', statusError as Error, {
                                    statusId: status.id,
                                    status: status.status,
                                    operation: 'processWebhookPayload:status_update'
                                });
                                // Continue with next status instead of throwing
                            }
                        }
                    }
                    ...
                    logError('Error processing webhook change', changeError as Error, {
                        operation: 'processWebhookPayload:change',
                        changeField: change.field
                    });
                    // Continue with next change instead of throwing
                }
            }
        }
    } catch (error) {
